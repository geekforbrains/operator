...
defaults:
  ...
  env_file: ".env"               # PATH + API keys (generated by `operator init`)
...
def init() -> None:
    ...
    # Generate .env with the user's current PATH
    env_file = home / ".env"
    if env_file.exists():
        ...
        env_content = (
            f"# Generated by `operator init` — captures your interactive shell's PATH\n"
            f"# so that subprocesses (hooks, skills, run_shell) can find your tools.\n"
            f"PATH={current_path}\n"
            f"\n"
            f"# API keys — uncomment and fill in as needed:\n"
