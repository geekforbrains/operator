...
_SETUP_TRANSPORTS: dict[str, SetupTransport] = {
    "slack": SetupTransport(
        ...
        identity_prompt="Your Slack user ID or mention",
        identity_help="Paste a Slack user ID like U123ABC45 or a mention like <@U123ABC45>.",
        secrets=(
            SetupSecret(
    ...
    *,
    ...
    return textwrap.dedent(f"""\
        ...
        defaults:
          ...
          env_file: ".env"               # API key defaults (generated by `operator init`)
...
def _normalize_slack_user_id(value: str) -> str:
    ...
    if not match:
        raise ValueError("Use a Slack user ID like U123ABC45 or a mention like <@U123ABC45>.")
...
def _resolve_secret(
    *,
    ...
    if not value and not force:
        value = env_file_values.get(env_var, "")
        source = str(env_file)
    if not value and not force:
        value = os.environ.get(env_var, "").strip()
        source = "shell environment"
    while not value:
        value = typer.prompt(prompt, hide_input=hidden).strip()
        source = "prompt"
        if not value:
            console.print(f"[red]{prompt} is required.[/red]")
    if warning_prefix and not value.startswith(warning_prefix):
        console.print(
            f"[yellow]Warning:[/yellow] {env_var} usually starts with [bold]{warning_prefix}[/bold]."
        )
    if source != "prompt":
...
def init() -> None:
    ...
    result = _scaffold_operator_home(OPERATOR_DIR)
...
def setup(
    ...
    slack_user: str | None = typer.Option(
        None,
        "--slack-user",
        help="Slack user ID or mention for your admin identity.",
    ),
    anthropic_api_key: str | None = typer.Option(
        None, "--anthropic-api-key", help="Anthropic API key to persist into ~/.operator/.env."
    ),
    ...
    result = _scaffold_operator_home(OPERATOR_DIR, emit_output=False)
    ...
    console.print(f"Default model: [bold]{_DEFAULT_MODEL}[/bold]\n")
    ...
    for secret in _CORE_SETUP_SECRETS + selected_transport.secrets:
        secrets[secret.env_var] = _resolve_secret(
            cli_value=secret_overrides.get(secret.env_var),
            env_var=secret.env_var,
            prompt=secret.prompt,
            env_file_values=env_file_values,
            env_file=result.env_file,
            hidden=secret.hidden,
            warning_prefix=secret.warning_prefix,
            force=force,
        )
